...
with tabs[0]:
    ...
    lsl = c1.number_input("LSL", value=3.0); target = c2.number_input("Target", value=3.5); usl = c3.number_input("USL", value=4.0)
    cap = analyze_capability(values, lsl=lsl, target=target, usl=usl)
    cols = st.columns(4)
    for col, label, value in zip(cols, ["Mean", "Sample STDEV", "Cp", "Cpk"], [f"{cap.mean:.6f}", f"{cap.sample_stdev:.6f}", f"{cap.cp:.3f}", f"{cap.cpk:.3f}"]): col.metric(label, value)
    st.warning(cap.status); st.line_chart(pd.DataFrame({"measurement_mm": values}))
